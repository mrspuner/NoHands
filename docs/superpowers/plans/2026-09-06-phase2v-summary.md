...
Посмотреть глазами: доли у решений, которые выглядят настоящими, должны быть заметно выше 0,4, у сомнительных — ниже. Если картина расходится с замером, порог правится в `~/.config/nohands/meetings.json`, а не в коде.
