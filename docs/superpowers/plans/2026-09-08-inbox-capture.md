...
**Первый захват из браузера — отдельный случай, и он важнее остальных.** Диалог автоматизации
поднимается один раз в жизни приложения, и порядок в коде специально сделан так, чтобы он
пришёл уже после копирования: текст к этому моменту снят, фокус может уезжать куда угодно.
Проверить, что после ответа на диалог входящее создалось целиком — с текстом, с именем
приложения и с адресом страницы.
...
Это единственное место ветки, до которого не дотягивается ни один тест: `dropDestination(for:
URL.self)` принимает то, что даёт перетаскивающее приложение, а Телеграм в принципе может
отдать не адрес файла, а обещание его подготовить. Если файл не приезжает — проверить, скачан
ли он в Телеграме до конца, и записать наблюдение: лечится это другим типом полезной нагрузки,
а не подкруткой окна.
...
- [ ] **Step 6б: Клавиша не заводит сама себя**

Приложение теперь само посылает Cmd+C с кодом клавиши, за которым следит его же перехватчик
событий. Расчёт в том, что посылаемое событие несёт ровно `.maskCommand`, а значит перехватчик
его не узнаёт и пропускает насквозь. Если расчёт неверен, один fn+C превратится в бесконечную
череду захватов.

Проверяется одним нажатием: после fn+C в `~/Inbox` появляется **ровно одна** папка, а не
несколько, и панель показывает одну строку, а не мигает. Отказ захвата в полёте ограничивает
ущерб одним лишним событием даже в худшем случае, но убедиться стоит глазами.
